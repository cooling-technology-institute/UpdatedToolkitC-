...
/*
 * New function to reset dialog to minimum (original) size.
 * David Lavo 06/19/03
 */

void cdxCSizingDialog::ControlInfo::Header::ResetWindowSize()
{
	ASSERT(IsValid());
	WINDOWPLACEMENT	wpl;
	if(!m_pWnd->GetWindowPlacement(&wpl))
	{
		...
		if(s.IsEmpty())
			s.Format(_T("<Address_0x%lx"),(UINT)this);
		TRACE1(_T("*** ERROR[cdxCSizingDialog::ControlInfo::Header::ResetWindowSize()]: GetWindowPlacement() failed for dialog with caption \"%s\" !\n"),(LPCSTR)s);
#endif
		return;
	}
	wpl.flags	=	0;
	m_pWnd->SetWindowPos(&wndBottom,	
...
cdxCSizingDialog::ControlInfo *cdxCSizingDialog::ControlInfo::Header::AddSzControlEx(CWnd & ctrl, BYTE dX1pcnt, BYTE dX2pcnt, BYTE dY1pcnt, BYTE dY2pcnt)
{
	...
	ControlInfo	*ci = 
		new ControlInfo(*this,ctrl,dX1pcnt,dX2pcnt,dY1pcnt,dY2pcnt);
...
bool cdxCSizingDialog::ControlInfo::Header::RestoreWindowPosition(const CString & profileSec, bool withState)
{
	...
	//
	ok = (AfxGetApp()->GetProfileInt(profileSec,pfEntry_IsDone,0) == const_IsDone);
	if (ok)
	{
...
cdxCSizingDialog::ControlInfo::~ControlInfo()
{
	...
	{
		if ((m_pPrev->m_pNext = m_pNext) != NULL)
...
void cdxCSizingDialog::ControlInfo::AdaptControlSize(register int deltaX, register int deltaY)
{
	...
	if(deltaX)
	{
		if ((i = m_dX1pcnt) != 0)			// NOTE exIgnore == 0
			x1	+=	(deltaX * i) / 100;
		if ((i = m_dX2pcnt) != 0)			// NOTE exIgnore == 0
			x2	+=	(deltaX * i) / 100;
	}

	if(deltaY)
	{
		if ((i = m_dY1pcnt) != 0)			// NOTE exIgnore == 0
			y1	+=	(deltaY * i) / 100;
		if ((i = m_dY2pcnt) != 0)			// NOTE exIgnore == 0
			y2	+=	(deltaY * i) / 100;
	}
...
BOOL cdxCSizingPropSheet::OnInitDialog(UINT addSzByPcnt) 
{
	...
	else
	{
		...
		if(ci)
		{
			...
			for(i=0; i<CPropertySheet::GetPageCount(); ++i)
				if ((ow = CPropertySheet::GetPage(i)) != (CWnd *)0)
	...
	{
		if ((w = CPropertySheet::GetDlgItem(psheetCtrls[i].id)) != (CWnd *)0)
		{
	...
	if (helpBut && 
		((w = m_Controls.GetSizeIconCtrl()) != (CWnd *)0))
...
BOOL cdxCSizingPropPage::OnEraseBkgnd(CDC* /* pDC */) 
{
	return FALSE;
}
